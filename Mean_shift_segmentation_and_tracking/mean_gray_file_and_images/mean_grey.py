# -*- coding: utf-8 -*-
from PIL import Image#NU
import numpy as np
import matplotlib.pyplot as plt
from scipy import ndimage#NU
from scipy.spatial import distance
from collections import deque#NU
...
#Image_Butterfly.jpg
arr = misc.imread('Image_Hill-House.jpg')
#arr=misc.imresize(arr,0.5)

#im = Image.open('hill_hist.jpg').convert('L')
#arr = np.array(im, dtype=int)

# 640x480x3 array
plot(arr,'original')
#arr=arr[200:300,200:300]
arr2 = misc.imread('Image_Hill-House.jpg') # 640x480x3 array
#arr2=misc.imresize(arr2,0.5)
#im = Image.open('hill_hist.jpg').convert('L')
#arr2 = np.array(im, dtype=int)


#arr2=arr2[200:300,200:300]
...
#3 dimentional feature vector array
vectors=np.empty([size,3], dtype=int)
count=0
for i in range(len(arr)):
    for j in range(len(arr[0])):
        vectors[count]=[arr[i][j],i,j]
        count=count+1
...
while(len(vectors)):
    try:                
        print 'Remaing feature vectore size'
        print len(vectors)
        for i in range(len(vectors)):
#****spacial and rande euclidean distance calculation            
            dsths = distance.euclidean(vectors[i][0:1],t_mean[0:1])
            dsthr = distance.euclidean(vectors[i][1:3],t_mean[1:3])
#            print 'here'
#            print dsths*dsthr
            prod=dsths*dsthr
#            print prod
...
#            if(prod<7000):
            if (dsths<hs and dsthr<hr):
                rem.append(i)
        print 'remove region pixel count'
        print len(rem)
#        print 'l_vec'
#        print len(vectors)
        if(len(vectors)==0):
...
#** Shift from of new Convergence point to previous one
        shift=distance.euclidean(t_mean[1:3],t_avg[1:3])
...
#*** Check if local maxima is reached else iterate again
        if(shift<iter and len(vectors)>1):
            print 'Low shift/ Local Maxima  - Remove this region of convergence'
#            if(len(rem)>0):
            if(1):
                no_boundary=0;
                if(no_boundary==0):
                    for key in rem:                    
                        arr2[vectors[key][1]][vectors[key][2]]=t_avg[0]
...
while(len(store_mean.keys())):
    ...
    for j in store_mean.keys():
        color = distance.euclidean(reg_mean[0:1],store_mean[j][0:1])
        space = distance.euclidean(reg_mean[1:3],store_mean[j][1:3])
...
#*** Select a proper parameter P so that regions smaller than P pixels will be eliminated.
    if(combined_region_pixels_count<merged_region_size_threshold):
        for eachkey in keys:        
            ...
            each_region=store_region[actual_key]
            for m in range(len(store_region[actual_key])):
                arr50[each_region[m][1]][each_region[m][2]]=arr[each_region[m][1]][each_region[m][2]]
...
#****Assign 𝐿𝑖 = {𝑝|𝐳𝑖 ∈ 𝐂𝑝} for each pixel 𝑖 = 1, … ,n 
    ...
    #*** New empty array will be filled in all the indices of the regions merged by ther averge of local maximas RGB value
                arr50[each_region[m][1]][each_region[m][2]]=compute_mean[0]
                empty1[each_region[m][1]][each_region[m][2]]=50
            del store_mean[actual_key]
        store_current_mean={}
    
    #*** Boundary of each reach marked by white color
        try:
            for qwe in range(len(empty1)-2):
                for rty in range(len(empty1[0])-2):
                    if empty1[qwe][rty]!=empty1[qwe][rty+1]:
                        arr60[qwe][rty]=255
                   #     arr60[qwe][rty-1]=255
                   #     arr60[qwe][rty+1]=255
                    if empty1[qwe][rty]!=empty1[qwe+1][rty]:
                        arr60[qwe-1][rty]=255
                 #       arr60[qwe][rty]=255
                  #      arr60[qwe+1][rty]=255
        except ValueError:
